...
const AdminDashboard = () => {
  ...
  const getActivityIcon = (actionType, description) => {
    if (actionType === "register") return { icon: faUserPlus, color: "text-blue-600 bg-blue-100 dark:text-blue-400 dark:bg-blue-800" };
    if (actionType === "approve") return { icon: faUserCheck, color: "text-green-600 bg-green-100 dark:text-green-400 dark:bg-green-800" };
    if (actionType === "decline") return { icon: faUserTimes, color: "text-red-600 bg-red-100 dark:text-red-400 dark:bg-red-800" };
    if (actionType === "create_event" || description.includes("Event")) return { icon: faCalendarPlus, color: "text-indigo-600 bg-indigo-100 dark:text-indigo-400 dark:bg-indigo-800" };
    if (actionType === "update") return { icon: faPen, color: "text-yellow-600 bg-yellow-100 dark:text-yellow-400 dark:bg-yellow-800" };
    if (actionType === "delete") return { icon: faTrash, color: "text-gray-600 bg-gray-100 dark:text-gray-400 dark:bg-gray-700" };
    return { icon: faUserPlus, color: "text-blue-600 bg-blue-100 dark:text-blue-400 dark:bg-blue-800" };
  };
      ...
      <main className="flex-1 p-6">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              >
                ...
                <p className="mt-2 text-3xl font-bold">{card.value ?? "Loading..."}</p>
                <p className={`text-xs mt-1 ${isDark ? "text-green-400" : "text-green-600"}`}>Live Data</p>
              </div>
            ))}
          </div>
          ...
          <div className={`p-6 rounded-lg shadow mb-8 transition-colors ${isDark ? "bg-gray-800 text-white" : "bg-white text-gray-900"}`}>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <YAxis stroke={isDark ? "#D1D5DB" : "#374151"} allowDecimals={false} />
                <Tooltip
                  contentStyle={{
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            {/* Recent Activity */}
            <div className={`p-6 rounded-lg shadow max-h-96 overflow-y-auto transition-colors ${isDark ? "bg-gray-800 text-white" : "bg-white text-gray-900"}`}>
              <h3 className="mb-4 text-lg font-semibold">Recent Activity</h3>
              <div className="space-y-4">
                {recentActivity.length > 0 ? (
                  recentActivity.map((log) => {
                    const { icon, color } = getActivityIcon(log.action_type, log.description);
                    return (
                      <div
                        key={log.id}
                  ...
                  <p className={`text-sm ${isDark ? "text-gray-400" : "text-gray-500"}`}>No recent activities.</p>
                )}
              </div>
            </div>
            ...
            <div className={`p-6 rounded-lg shadow transition-colors ${isDark ? "bg-gray-800 text-white" : "bg-white text-gray-900"}`}>
              ...
              <div className="space-y-2">
                {["/admin/create-user","/admin/create-event","/admin/reports"].map((path, i) => (
                  <button
                    key={i}
                  ...
                  >
                    {path.includes("user") ? "Create User" : path.includes("event") ? "Create Event" : "Generate Report"}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex justify-center lg:col-span-2">
              <div className={`p-6 rounded-lg shadow flex flex-col items-center w-full max-w-sm transition-colors ${isDark ? "bg-gray-800 text-white" : "bg-white text-gray-900"}`}>
                <h3 className={`text-sm font-medium ${isDark ? "text-green-400" : "text-green-500"}`}>Online Users</h3>
                {onlineUsers ? (
                  <ul className="mt-2">
                    {onlineUsers.map((user) => (
                      <li key={user.id} className={`text-sm ${isDark ? "text-gray-200" : "text-gray-800"}`}>
                        {user.firstname || "Unknown"} {user.middle_initial ? `${user.middle_initial}. ` : ""}{user.lastname || "User"} - {user.role || "unknown"}
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className={`text-sm mt-2 ${isDark ? "text-gray-400" : "text-gray-500"}`}>Loading...</p>
                )}
              </div>
            </div>
          </div>
