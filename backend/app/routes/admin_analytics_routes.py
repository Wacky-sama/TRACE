...
def get_admin_analytics(
    ...
    - department (to filter alumni/GTS data)
    """
    ...
    try:
        ...
        if start_year:
            gts_filters.append(extract("year", GTSResponses.created_at) >= start_year)
        if end_year:
            gts_filters.append(extract("year", GTSResponses.created_at) <= end_year)
        ...
        # ======== BASIC COUNTS ========
        total_alumni = db.query(func.count(User.id)).filter(*filters).scalar()
        active_alumni = db.query(func.count(User.id)).filter(User.is_active == True, *filters).scalar()
        pending_approvals = db.query(func.count(User.id)).filter(User.is_approved == False, *filters).scalar()
        ...
        recent_logins = (
            ...
            .filter(User.last_login >= last_7_days, *filters)
            .scalar()
        )
        ...
        employment_trend = (
            db.query(
                extract("year", GTSResponses.created_at).label("year"),
        ...
        event_trend = (
            db.query(
                extract("month", Event.start_date).label("month"),
        ...
        gts_by_department = (
            db.query(User.department, func.count(GTSResponses.id))
        ...
        # ======== RECENT ACTIVITIES ========
        recent_activities = (
            db.query(ActivityLog.description, ActivityLog.timestamp)
